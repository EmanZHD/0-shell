   ...
   let mut is_err = false;
        ...
        for ( i , element) in files.iter().enumerate()  {
            if i == files.len()-1{
                ...
                if element.chars().nth(0) == Some('"') &&  element.chars().nth(element.len()-1) == Some('"') {
                                    ...
                                    if let Some(file_name) =  path.file_name() {
                                      ...
                                      if let Some(file_name) =  path.file_name() {
                                        ...
                                        if file_name.to_string_lossy().ends_with(suffix){
                                             ...
                                             println!("cp: -r not specified; omitting directory '{}'" , file_name.display());
                                             break
